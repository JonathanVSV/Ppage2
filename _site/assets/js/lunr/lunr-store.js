var store = [{
        ...
        "excerpt":"Working with LiDAR data in R For this example we are going to use the lidR package. Additionally, we are going to load raster rgdal and plyr. The first thing is to load the LiDAR data as a catalog and load a shapefile with the areas of interest (that can...","categories": ["blog"],
        "tags": ["post","r","LiDAR","point cloud","raster"],
        ...
        "excerpt":"Beautiful plots in R using ggplot2 The purpose of this post is to show how to use the basic syntax of ggplot2, do some of the most common types of plots, show some customizations and facets. library(rgbif) library(skimr) library(dplyr) library(tidyr) library(rnaturalearth) library(sf) library(tmap) The first step will be to set...","categories": ["blog"],
